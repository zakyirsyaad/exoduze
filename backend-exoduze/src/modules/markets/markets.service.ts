...
export class MarketsService {
          ...
          market_stats: {
            ...
            support_share_pct:
              totalFollowerStakeUnits === 0n
                ? 0
                : Number((followerStakeUnits * 10_000n) / totalFollowerStakeUnits) /
                  100,
          },
        };
      }),
      ...
      data: {
        market: {
          ...
          resolution: {
            ...
            oracle_status: resolutionProposal?.status ?? resolution?.status ?? "pending",
            final_outcome:
              resolution?.outcome ??
  ...
  async resolveMarket(marketIdOrSlug: string, input: MarketResolveInput) {
    ...
    try {
      ...
      const settlement = await this.settleMarketPayouts(
        client,
        market,
        input,
      );
  ...
  private async settleMarketPayouts(
    ...
    const positionRows = await queryRows<SettlementPositionRow>(
      ...
      `
        ...
        GROUP BY up.wallet_identity_id, up.market_agent_id, ma.final_decision_side
    ...
    const positionWithMissingSide = positions.find(
      (position) =>
        position.stakeUnits > 0n &&
        !["YES", "NO"].includes(position.final_decision_side ?? ""),
    );

    if (positionWithMissingSide) {
      throw new HttpError(
        409,
        "POSITION_DECISION_MISSING",
        "All staked market agents must have a final decision before payout settlement.",
      );
    }

    const totalStakeUnits = positions.reduce(
      (total, position) => total + position.stakeUnits,
      0n,
    );
    const winningStakeUnits = positions.reduce(
      (total, position) =>
        position.final_decision_side === input.outcome
          ? total + position.stakeUnits
          : total,
      0n,
    );
    ...
    if (winningStakeUnits > 0n) {
      for (const position of positions) {
        ...
        const winningsShare =
          (losingStakeUnits * position.stakeUnits) / winningStakeUnits;
        const grossUnits = position.stakeUnits + winningsShare;
        ...
        await client.query(
          ...
          [
            ...
            market.id,
            position.market_agent_id,
            this.formatDecimalUnits(grossUnits),
    ...
    return {
      ...
      winning_stake_usdc: this.formatDecimalUnits(winningStakeUnits),
      losing_stake_usdc: this.formatDecimalUnits(losingStakeUnits),
